...
def create_training_options():
    ...
    parser.add_argument("--trigger-size",   type=int,   default=32,          help="size of the trigger square")
    parser.add_argument("--backdoor-dir",   type=Path,  default="/BD_dir1",  help="path to backdoor dataset 1")
    parser.add_argument("--backdoor-dir2",  type=Path,  default="/BD_dir2",  help="path to backdoor dataset 2")
    parser.add_argument("--backdoor-dir3",  type=Path,  default="/BD_dir3",  help="path to backdoor dataset 3")
    parser.add_argument("--backdoor-batch", type=int,   default=10,          help="batch-size of backdoor image to calculate the optimal transport map")
    parser.add_argument("--combine",        action="store_true",             help="determine the combination of models")
    parser.add_argument("--wrs",            action="store_true",             help="determine the regularization of weight")
    parser.add_argument("--ckpt1",          type=str,   default=None,        help="resumed checkpoint name")
    parser.add_argument("--ckpt2",          type=str,   default=None,        help="resumed checkpoint name")
    parser.add_argument("--ckpt3",          type=str,   default=None,        help="resumed checkpoint name")
    parser.add_argument("--ckpt4",          type=str,   default=None,        help="resumed checkpoint name")
    parser.add_argument("--ckpt5",          type=str,   default=None,        help="resumed checkpoint name")
    ...
    if opt.ckpt1 is not None:
        ckpt_file = RESULT_DIR / opt.ckpt1 / "latest.pt"
        assert ckpt_file.exists()
        opt.load1 = ckpt_file
    else:
        opt.load1 = None

    if opt.ckpt2 is not None:
        ckpt_file = RESULT_DIR / opt.ckpt2 / "latest.pt"
        assert ckpt_file.exists()
        opt.load2 = ckpt_file
    else:
        opt.load2 = None
    
    if opt.ckpt3 is not None:
        ckpt_file = RESULT_DIR / opt.ckpt3 / "latest.pt"
        assert ckpt_file.exists()
        opt.load3 = ckpt_file
    else:
        opt.load3 = None

    if opt.ckpt4 is not None:
        ckpt_file = RESULT_DIR / opt.ckpt4 / "latest.pt"
        assert ckpt_file.exists()
        opt.load4 = ckpt_file
    else:
        opt.load4 = None

    if opt.ckpt5 is not None:
        ckpt_file = RESULT_DIR / opt.ckpt5 / "latest.pt"
        assert ckpt_file.exists()
        opt.load5 = ckpt_file
    else:
        opt.load5 = None
...
def main(opt):
    ...
    backdoor_dataset  = imagenet.build_backdoor_dataset(opt, log, train=True, fn=opt.backdoor_dir)
    backdoor_dataset2 = imagenet.build_backdoor_dataset(opt, log, train=True, fn=opt.backdoor_dir2)
    backdoor_dataset3 = imagenet.build_backdoor_dataset(opt, log, train=True, fn=opt.backdoor_dir3)
    ...
    run = Runner(opt, log)
    run.train_combination(opt, train_dataset, val_dataset, backdoor_dataset, backdoor_dataset2, backdoor_dataset3, corrupt_method)
    log.info("Finish!")
